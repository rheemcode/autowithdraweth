...
const provider =
  ethers.providers.AlchemyProvider.getWebSocketProvider("homestead");

const depositWallet = new ethers.Wallet(process.env.PRIVATE_KEY, provider);

const main = async () => {
  const depositWalletAddress = await depositWallet.getAddress();
  const _currentBalance = await depositWallet.getBalance("latest");
  console.log(utils.formatEther(_currentBalance));
